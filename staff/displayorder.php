...
<?php
	...
	// no mostrar ninguna cuando esté abierta /displayorder.php
	if(empty($_GET['cmd'])) 
		die(); 

	//muestra la lista de pedidos actual para la gestión de la cocina
				...
				if ($orderResult->num_rows == 0) {
					...
					while($orderRow = $orderResult->fetch_array(MYSQLI_ASSOC)) {
						...
						if ($currentspan == $rowspan) {
							...
							//opciones basadas en el estado del pedido
							switch ($orderRow['status']) {
								case 'waiting':
	...
	//muestra la lista de pedidos listos actuales en el índice del personal
